...
function main() {
    ...
    const exchange2 = new ExchangePlatform2();
    const display = new PriceDisplay();
    exchange2.attach(display);

    exchange2.setPrice(25000);

    const marketData = new MarketData(25000);
    const transaction = new Transaction(new StrategyContext());

    const conservativeStrategy = new ConservativeStrategy();
    const aggressiveStrategy = new AggressiveStrategy();

    const strategyContext = new StrategyContext(conservativeStrategy);
    strategyContext.executeStrategy(transaction, marketData);

    marketData.setPrice(26000);
    strategyContext.setStrategy(aggressiveStrategy);
    strategyContext.executeStrategy(transaction, marketData);
